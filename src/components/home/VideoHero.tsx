  ...
  return (
    <div>
      ...
      <section className="relative h-[560px] md:h-[620px] overflow-hidden bg-gray-900">
        ...
        {/* Service quick-nav strip — desktop only, pinned to bottom of hero */}
      ...
      {/* Service quick-nav strip — mobile only, below the video as a dark bar */}
      <div className="md:hidden bg-[#0a0a0f] px-4 py-4">
        <div className="flex flex-wrap gap-2 justify-center">
          {serviceLinks.map((s) => (
            <Link
              key={s.href}
              href={s.href}
              className="text-sm font-semibold text-white bg-white/10 hover:bg-accent border border-white/25 hover:border-accent px-4 py-2 rounded-full transition-all duration-200"
            >
              {s.label}
            </Link>
          ))}
